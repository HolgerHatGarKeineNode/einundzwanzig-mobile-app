...
- [ ] 1.21 **launchMode-Fix persistent machen:** `nativephp/` ist ephemer — `php artisan native:install --force` überschreibt das Manifest (normales `native:run`/Gradle patcht nur in-place, Fix bleibt erhalten). Optionen: (a) Upstream-Issue/PR bei NativePHP (singleTask als Default oder config-Key), (b) Post-Install-Hook/Skript, das `singleTop`→`singleTask` ersetzt, (c) manuell nach jedem `native:install` neu anwenden (dokumentiert in Memory). Außerdem offen: Lightning-Login weiterhin ungetestet (kein Wallet); Emulator-Gegentest des Auth-Flows mit singleTask (optional — Pixel 8 ist Referenz).
